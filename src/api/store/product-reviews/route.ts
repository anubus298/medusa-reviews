...
import {
  defaultStoreProductReviewFields,
  UpsertProductReviewsSchema,
} from "./middlewares";
...
export const GET = async (
  ...
  const { data: product_reviews, metadata = { count: 0, skip: 0, take: 0 } } =
    await query.graph(
      {
...
export const POST = async (
  req: AuthenticatedMedusaRequest<UpsertProductReviewsSchema>,
  res: MedusaResponse
) => {
  const query = req.scope.resolve(ContainerRegistrationKeys.QUERY);
  const { reviews } = req.validatedBody;

  const { result } = await upsertProductReviewsWorkflow(req.scope).run({
    input: { reviews },
  });
