import { ChatBubble, CheckCircle, Eye, Plus } from "@medusajs/icons";
import {
  Button,
  DataTable,
  Heading,
...
import {
  useAdminListProductReviews,
  useAdminUpdateProductReviewStatusMutation,
...
const getColumns = (
  ...
  const updateStatusActions = (review: AdminProductReview) => {
    return PRODUCT_REVIEW_STATUSES.filter(
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex items-center gap-4 py-2 w-full min-w-[200px] max-w-[300px]">
            {product.thumbnail ? (
              <img
                className="h-8 w-8 flex-shrink-0 rounded-md my-1"
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex flex-col gap-2 my-2 min-w-[200px] max-w-[400px]">
            ...
            <p className="text-gray-700 whitespace-normal break-words line-clamp-3">
              {content}
            </p>
          </div>
        );
  ...
  return showColumns
    .map((c) => allColumns.find((column) => column.id === c))
    .filter((c) => !!c);
};
...
export const ProductReviewDataTable = ({
  ...
  defaultQuery: AdminListProductReviewsQuery;
  showColumns?: string[];
  product_id?: string;
  order_id?: string;
  ...
  const { data, isLoading } = useAdminListProductReviews({
    ...query,
    limit: defaultLimit,
  });
  ...
  const table = useDataTable({
    columns: getColumns(showColumns, {
      setSelectedReview,
      setSelectedReviewForDetails,
      updateStatus,
    }),
    data: data?.product_reviews ?? emptyData,
    ...
    <>
      ...
      {selectedReviewForDetails && (
        <ProductReviewDetailsDrawer
          ...
          setOpen={(open) =>
            setSelectedReviewForDetails(open ? selectedReviewForDetails : null)
          }
        />
      )}
